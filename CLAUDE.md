...
## قواعد استخدام Dhee (إلزامية في كل جلسة)

> خالد يشترط استخدام Dhee في كل محادثة وكل مشروع بشكل تلقائي دون انتظار طلب.

1. **عند بداية كل محادثة:** استدعِ `mcp__dhee__context` فوراً قبل أي عمل.
2. **كل أمر Shell:** استخدم `mcp__dhee__dhee_bash` بدلاً من Bash مباشرةً.
3. **كل نتيجة طويلة:** مرّرها عبر `mcp__dhee__dhee_agent` بدلاً من قراءتها مباشرة.
4. **حفظ المعلومات:** استخدم `mcp__dhee__remember` لأي تفضيل أو قرار مهم.
5. **استرجاع المعلومات:** استخدم `mcp__dhee__recall` قبل البدء في أي مهمة.
6. **عند نهاية كل محادثة:** استدعِ `mcp__dhee__checkpoint` لحفظ حالة الجلسة والمهام المتبقية.
...
5. **الخطوط:** الخط المعتمد هو 'Cairo' بأوزان تتراوح بين 300 و 900.
