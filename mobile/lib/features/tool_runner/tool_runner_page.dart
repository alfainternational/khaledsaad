...
class ToolRunnerPage extends StatelessWidget {
      ...
      body: Obx(() {
        ...
        if (form == null || form.modes.isEmpty) {
          return AppStateView.empty(
              icon: Icons.build_outlined, title: 'لا يوجد نموذج لهذه الأداة');
        }
        return ListView(
          ...
          children: [
              ...
              children: [
                Expanded(
                  child: Obx(() => OutlinedButton.icon(
                        ...
                        label: Text(c.isAnalyzing.value
                            ? 'جارٍ التحليل...'
                            : 'حلّل إجاباتي'),
                      )),
                ),
                ...
                Expanded(
                  child: Obx(() => OutlinedButton.icon(
                        onPressed: c.isSuggesting.value ? null : c.suggestFields,
                        ...
                        label: Text(c.isSuggesting.value
                            ? 'جارٍ الاقتراح...'
                            : 'اقترح لي'),
                      )),
                ),
            ...
            Obx(() => FilledButton.icon(
                  ...
                  label: Text(c.isRunning.value ? 'جارٍ التشغيل...' : 'تشغيل الأداة'),
                )),
            const SizedBox(height: 20),
            Obx(() {
              final result = c.result.value;
              if (result == null) return const SizedBox.shrink();
              return ToolResultView(result: result);
            }),
          ],
        );
      }),
    ...
    return Obx(() {
      ...
      return Column(
        ...
        children: [
          ...
          ...mode.fields.map((f) => DynamicFormField(
                key: ValueKey(
