...
class PublicProfessionalPagesTest extends TestCase
{
    ...
    public function test_professional_profile_is_complete_crawlable_and_uses_verified_data(): void
    {
        ...
        $this->get(route('profile'))
            ...
            ->assertSee('أطوّر وأنفّذ استراتيجيات تسويق رقمي متكاملة')
            ->assertSee('جامعة النيلين')
            ->assertSee('إدارة المشاريع الاحترافية PMP')
            ->assertSee('Certificate of completion: Introduction to Claude Cowork')
            ->assertSee('التسويق الرقمي - إدارة الحملات الرقمية المدفوعة')
            ->assertSee('مُعرّف الاعتماد:')
            ->assertSee('vpbuqq7qdqvp')
            ->assertSee('أمتلك خبرة تتجاوز عشر سنوات')
            ->assertDontSee('يمتلك خالد سعد')
            ->assertSee('تحسين نتائج محركات البحث SEO')
            ->assertSee('application/ld+json', false)
            ->assertSee('"hasOccupation"', false)
            ->assertSee('"hasCredential"', false)
