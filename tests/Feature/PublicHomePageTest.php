...
class PublicHomePageTest extends TestCase
{
    ...
    public function test_home_page_presents_the_full_marketing_journey_and_verified_profile(): void
    {
        ...
        $this->get(route('home'))
            ...
            ->assertSee('خالد سعد | اعرف من أين تبدأ في تسويق مشروعك')
            ->assertSee('جرّب الآن بدون حساب')
            ->assertSee('أي واحدة من هذه تحصل معك؟')
            ->assertSee('بماذا أقدر أساعدك؟')
            ->assertSee('أي حالة من هذه تشبه وضعك؟')
            ->assertSee('شكل النتيجة التي ستستلمها')
