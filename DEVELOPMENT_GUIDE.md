# DEVELOPMENT_GUIDE.md — دليل التطوير الكامل للمنصة التسويقية

> هذا الملف هو دليل التطوير الكامل للمشروع، وموجّه لأي مطور أو وكيل ذكي أو عضو فريق يعمل على هذه المنصة. يجب قراءته كاملاً قبل إجراء أي تعديل على الكود أو البنية.
>
...
>
> **ملاحظة تنظيمية:** هذا الملف نسخة تطويرية كاملة مخصصة للرجوع البشري المباشر داخل المشروع، ومحتواه متوافق مع المرجع المعماري الأساسي في `CLAUDE.md`.
...
/onboarding/context           → جمع السياق المرن
/onboarding/who-are-you       → شاشة تحديد نوع المستخدم (اختيارية)
/onboarding/your-goal         → شاشة تحديد الهدف (اختيارية)
/onboarding/suggested-path    → اقتراح المسار
...
- واضحة ومباشرة، تخاطب بـ"أنت"، تركز على النتيجة.
...
GET    /api/v1/workspaces/{ws_id}/projects/{proj_id}/tools/{tool_code}
POST   /api/v1/workspaces/{ws_id}/projects/{proj_id}/tools/{tool_code}/run
...
### قواعد التقسيم
- **Domain/** لا يعتمد على `Illuminate\Http`. نقي من HTTP — لذلك يمكن إعادة استخدامه في Jobs و Commands و API.
...
1. **اقرأ الطبقات الثلاث كاملة** قبل فتح IDE.
2. **لا تكتب منطق أعمال داخل Controller أو Model.** المنطق في Services/Actions داخل `Domain/` أو `Application/`.
3. **كل استعلام Eloquent يجب أن يُفلتَر بـ `workspace_id`** — استخدم global scope أو service wrapper.
4. **لا تضف ميزة بدون:** Feature Flag + Entitlement key + Policy + Migration + Test.
5. **لا تضع منطقاً حساساً في Blade.** `@can` و `@feature` فقط كدرع ظاهري.
6. **لا تستورد حزمة Admin panel ثقيلة** تفرض هيكلها.
7. **كل AI call يمر بـ Queue.**
8. **كل عنصر ظاهر خارجياً يملك `public_id`.**
9. **JSON فقط للمرن — لا للحقول القابلة للفلترة.**
10. **عند إضافة Module جديد:** مرّ بالـ lifecycle الأربع (Hidden → Internal → Beta → GA).
11. **اختبر على workspace من نوع `agency`** حتى لو الميزة غير مفعّلة للوكالات.
12. **كل PR يحدّث الوثيقة** إذا عدّل قراراً معمارياً.
