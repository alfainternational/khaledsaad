...
<div class="actions"><a class="btn btn--secondary" href="{{ route('admin.consultations.simulate', $version) }}">حاكِ النطاق</a>@if($version->status === 'draft')<form method="POST" action="{{ route('admin.consultations.publish', $version) }}" onsubmit="return confirm('نشر هذا الإصدار وقفله؟')">@csrf<button class="btn btn--primary">تحقق وانشر</button></form>@endif</div>
