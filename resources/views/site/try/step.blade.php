    ...
    <main id="main-content" class="try-shell">
        <div class="container try-layout layout-page layout-page--form">
            ...
            <form method="POST" action="{{ route('try.step.save', [$run['uuid'], $step_number]) }}" class="form form--wide">
                ...
                @if ($known !== [])
                    {{-- نفس صياغة معالج الحساب: الزائر لا يشعر أنه في مسار ثانٍ. --}}
                ...
                <div class="form__actions">
                    ...
                    <button type="submit" class="btn btn--primary">
                        {{ $next_step === null ? 'اعرض نتيجتك الأولية' : 'التالي' }}
                    </button>
                </div>
            </form>

            {{--
                مكان الحفظ يُقال صراحةً: الزائر بلا حساب يسأل نفسه «أين تذهب
                إجاباتي؟» قبل أن يكتب، وإجابة صريحة هنا أرخص من تركه يخمّن.
            --}}
            <p class="try-note">
                تُحفظ إجاباتك على هذا الجهاز ثلاثين يومًا. إن أنشأت حسابًا لاحقًا انتقلت معك كما هي.
            </p>
        </div>
    </main>
