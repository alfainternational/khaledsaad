 ...
 *
 * تمييز جوهري بين مرتبتي المصدر:
 * - business_model: ما صرّح به المستخدم (أو ما حفظته أداة سابقة). يقين،
 *   فيصلح لإظهار أسئلة إلزامية.
 * - sector المعلن (projects.sector): اختيار المستخدم من قائمة مغلقة — يقين
 *   بنفس المرتبة، وعليه بُني التخصص القطاعي الثلاثي (مواصفة
 *   SECTOR-SPECIALIZATION.md).
 * - sector المستنتَج من نص الوصف: ترجيح، فيصلح لأسئلة عمق اختيارية فقط —
 *   لا نُلزم مستخدمًا بسؤال بُني على تخمين قد يكون خاطئًا. يعمل احتياطًا
 *   حين لا إعلان (مشاريع قديمة أو «قطاع آخر»).
 */
class ProjectContextResolver
{
    ...
    private const SECTOR_HINTS = [
        // قطاعا تخصص بلا إعلان: مشروع قديم كتب «مدرسة أهلية» يستحق أسئلة
        // قطاعه وإن لم يمرّ على المنتقي. قبل العام لأن كلماتهما أكثر تمييزًا.
        Sector::EDUCATION => ['تعليم', 'مدرسة', 'مدارس', 'جامعة', 'كلية', 'معهد', 'روضة', 'حضانة', 'أكاديمية', 'تحفيظ'],
        Sector::REAL_ESTATE => ['عقار', 'عقارات', 'وساطة عقارية', 'تطوير عقاري', 'إدارة أملاك', 'وحدات سكنية', 'مخطط', 'كمباوند'],
        'saas' => ['تطبيق', 'منصة', 'اشتراك شهري', 'برمجيات', 'نظام إدارة', 'سوفتوير'],
        Sector::ECOMMERCE => ['متجر', 'الشحن', 'التوصيل', 'سلة', 'منتجات', 'طلبات أونلاين'],
    ...
    {
        // المعلن يحسم: اختيار من قائمة مغلقة يقين لا يُنازعه استدلال نصي.
        // «قطاع آخر» ليس قطاعًا بل باب — نستدلّ خلفه كما مع القدامى.
        if (Sector::isSpecialized($project->sector)) {
