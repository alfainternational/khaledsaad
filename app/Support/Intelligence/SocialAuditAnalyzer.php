    ...
    public function analyze(array $socialLinks, ?string $primaryDomain = null, array $manualProfiles = []): array
    {
        ...
        foreach ($requestedLinks as $url) {
            ...
            if (! ($response['ok'] ?? false)) {
                ...
                $findings[] = $this->finding(
                    ...
                    0.45,
                    6,
                    'تعذّرت قراءة صفحة '.$network.' العامة',
                    'بيانات الحساب غير متاحة مباشرة من صفحته العامة، لذلك التقييم هنا محدود الثقة. سبب التعذّر: '.($response['error'] ?? 'غير معروف'),
            ...
            if (! $clarity) {
                $findings[] = $this->finding(
                    ...
                    0.70,
                    8,
                    'رسالة حسابك على '.$network.' غير واضحة بما يكفي',
                    'التعريف العام للحساب لا يوضّح لمن هذا الحساب وما النتيجة التي يقدّمها.',
                    'أعد كتابة التعريف ليشرح لمن تتوجّه والنتيجة التي تقدّمها وطريقة التواصل في جملة مباشرة.',
                    $url,
                );
            }
            ...
            if (! $hasCta) {
                $findings[] = $this->finding(
                    ...
                    0.63,
                    7,
                    'الحساب لا يوجّه إلى خطوة واضحة',
                    'الحساب موجود لكنه لا يدفع المتابع بوضوح إلى التواصل أو الحجز أو زيارة الموقع.',
                    'أضف زر إجراء أو دعوة ثابتة في التعريف وفي المنشور المثبّت أو الروابط الأساسية.',
                    $url,
                );
            }
            ...
            if (! $mentionsDomain && $primaryDomain) {
                $findings[] = $this->finding(
                    ...
                    0.58,
                    6,
                    'الربط بين حساباتك وموقعك ضعيف',
                    'لم يظهر رابط واضح من الحساب إلى موقعك، وهذا يضعف رحلة تحويل المتابع إلى عميل.',
                    'اربط الحساب بموقعك الرسمي أو بصفحة واضحة تعرض نفس العرض وتدفع لاتخاذ خطوة.',
                    $url,
                );
            }
        }
        ...
        if ($requestedProfileSources === 0) {
            $findings[] = $this->finding(
                ...
                0.95,
                15,
                'لا توجد روابط حسابات تواصل اجتماعي رسمية مضافة للمشروع',
                'غياب الروابط الرسمية يمنعنا من قياس حضورك والربط بين قنواتك.',
                'أضف روابط حساباتك الرسمية، أو اترك الحقل فارغاً عمداً إذا كانت خطتك بلا تواصل اجتماعي.',
                null,
            );
            ...
            $findings[] = $this->finding(
                ...
                0.55,
                4,
                'حضورك على التواصل الاجتماعي محدود بقناة واحدة',
                'وجود قناة واحدة لا يعني ضعفاً دائماً، لكنه يقلّل وضوح صورتك التسويقية.',
                'تأكد أن القناة الحالية تكفي فعلاً لجمهورك، أو وسّع حضورك بشكل مدروس عند الحاجة.',
